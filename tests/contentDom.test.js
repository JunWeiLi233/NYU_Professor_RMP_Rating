...
describe("Albert content DOM injection", () => {
  ...
  it("marks injected styles with the extension version for live Albert checks", () => {
    ...
    expect(style.dataset.nyuRmpVersion).toBe("0.1.11");
    expect(style.textContent).toContain("NYU Albert RMP Ratings v0.1.11");
    expect(style.textContent).toContain("--nyu-rmp-extension-version: \"0.1.11\"");
  });
  ...
  it("marks injected rating roots and cards with the extension version", async () => {
    ...
    expect(document.querySelector(".nyu-rmp-rating-root").dataset.nyuRmpVersion).toBe("0.1.11");
    expect(document.querySelector(".nyu-rmp-card").dataset.nyuRmpVersion).toBe("0.1.11");
  });
        ...
        <tbody>
          <tr id="legacy-select-row">
            ...
            <td data-nyu-rmp-rating-cell="true">
              <div class="nyu-rmp-rating-root is-cell-mounted" data-nyu-rmp-version="0.1.11">
                <article class="nyu-rmp-card rating-poor" data-nyu-rmp-requested-name="Chee Keng Yap" data-nyu-rmp-version="0.1.11">
  ...
  it("does not treat SCA Course Search notes about Faculty Electives as professor names", async () => {
    document.body.innerHTML = `
      <div id="win0divNYU_CLS_DTL_CLASS_NBR$52" class="ps_box-group psc_layout psc_num">
        <div id="win0divNYU_CLS_DERIVED_HTMLAREA3$52" class="ps_box-htmlarea">
          <div class="ps-htmlarea">
            <table>
              <tbody>
                <tr>
                  <td>
                    <div><b>SCA-UA 366</b> | 4 units</div>
                    <div><span>Class#:</span> 6690</div>
                    <br>09/02/2026 - 12/14/2026 Tue,Thu 4.55 PM - 6.10 PM at 40 W 4th St Room LC5 with Ouyang, Elizabeth
                    <br><span class="boldText"><b>Notes: </b></span>
                    Spaces are reserved for majors/minors in the 7 SCA programs the first week of registration.
                    Counts as SCA Faculty Elective for these majors/minors: Africana, American,
                    Asian/Pacific/American, Latino, and SCA.
                    <br><a href="https://example.edu/bookstore">Visit the Bookstore</a>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="ps_box-button" id="win0divSELECT_BUTTON$52">
          <span id="SELECT_BUTTON$span$52" class="ps-button-wrapper">
            <a id="SELECT_BUTTON$52" class="ps-button" role="button">Select Class #6690</a>
          </span>
        </div>
      </div>
    `;
    const lookupProfessor = vi.fn(async (name) => ({
      ...
      difficulty: 2.8,
      ratingsCount: 18,
      topComments: [`${name} SCA result comment.`],
      url: "https://www.ratemyprofessors.com/professor/419998",
    }));

    await Promise.all(scanAlbertPageOnce({ document, lookupProfessor }).pendingLookups);

    const htmlArea = document.getElementById("win0divNYU_CLS_DERIVED_HTMLAREA3$52");
    const selectButtonContainer = document.getElementById("win0divSELECT_BUTTON$52");
    const ratingRoot = selectButtonContainer.nextElementSibling;
    const cards = Array.from(document.querySelectorAll(".nyu-rmp-card"));
    expect(htmlArea.querySelector(".nyu-rmp-rating-root")).toBeNull();
    expect(ratingRoot.className).toBe("nyu-rmp-rating-root");
    expect(cards).toHaveLength(1);
  ...
  it("removes stale Course Search note ratings and remounts only the meeting-line instructor", async () => {
    document.body.innerHTML = `
      <div id="win0divNYU_CLS_DTL_CLASS_NBR$52" class="ps_box-group psc_layout psc_num">
        <div id="win0divNYU_CLS_DERIVED_HTMLAREA3$52" class="ps_box-htmlarea" data-nyu-rmp-processed="true">
          <div>
            ...
            <div class="nyu-rmp-rating-root" data-nyu-rmp-version="0.1.11">
              <article class="nyu-rmp-card" data-nyu-rmp-requested-name="Elective For These Majors" data-nyu-rmp-version="0.1.11">Bogus note match</article>
            </div>
          </div>
        </div>
        <div class="ps_box-button" id="win0divSELECT_BUTTON$52">
    ...
    const lookupProfessor = vi.fn(async (name) => ({
      ...
      difficulty: 2.8,
      ratingsCount: 18,
      topComments: [`${name} repaired SCA result.`],
      url: "https://www.ratemyprofessors.com/professor/419998",
    }));
    ...
    const selectButtonContainer = document.getElementById("win0divSELECT_BUTTON$52");
    const cards = Array.from(document.querySelectorAll(".nyu-rmp-card"));
    expect(htmlArea.dataset.nyuRmpProcessed).toBeUndefined();
    expect(htmlArea.querySelector(".nyu-rmp-rating-root")).toBeNull();
  ...
  it("replaces stale under-button Course Search roots with the current result-card layout", async () => {
    document.body.innerHTML = `
      <div id="win0divNYU_CLS_DTL_CLASS_NBR$52" class="ps_box-group psc_layout psc_num">
        <div id="win0divNYU_CLS_DERIVED_HTMLAREA3$52" class="ps_box-htmlarea">
          <div><b>SCA-UA 366</b> | 4 units</div>
          <div>09/02/2026 - 12/14/2026 Tue,Thu 4.55 PM - 6.10 PM at Tisch Hall with Ouyang, Elizabeth Notes: Counts as SCA Faculty Elective for these majors/minors: Africana.</div>
        </div>
        <div class="ps_box-button" id="win0divSELECT_BUTTON$52" data-nyu-rmp-processed="true" data-nyu-rmp-select-button-rating="true">
          <a id="SELECT_BUTTON$52" class="ps-button" role="button">Select Class #6690</a>
        </div>
        <div class="nyu-rmp-rating-root" data-nyu-rmp-version="0.1.6">
          <article class="nyu-rmp-card" data-nyu-rmp-requested-name="Elective For These Majors" data-nyu-rmp-version="0.1.6">Old stale card</article>
        </div>
      </div>
    `;
    const lookupProfessor = vi.fn(async (name) => ({
      ...
      difficulty: 2.8,
      ratingsCount: 18,
      topComments: [`${name} replaced stale root.`],
      url: "https://www.ratemyprofessors.com/professor/419998",
    }));
    ...
    expect(selectButtonContainer.dataset.nyuRmpSelectButtonRating).toBe("true");
    expect(selectButtonContainer.nextElementSibling.className).toBe("nyu-rmp-rating-root");
    expect(selectButtonContainer.nextElementSibling.dataset.nyuRmpVersion).toBe("0.1.11");
    expect(cards).toHaveLength(1);
    expect(cards[0].dataset.nyuRmpRequestedName).toBe("Elizabeth Ouyang");
  ...
  it("keeps Albert gridcell instructor text readable when marking it processed", async () => {
    ...
    expect(originalContent.dataset.nyuRmpVersion).toBe("0.1.11");
    ...
    document.body.innerHTML = `
      <div role="row">
        <div role="gridcell" id="grid-instructor" data-nyu-rmp-processed="true">
          ...
          <div class="nyu-rmp-rating-root is-cell-mounted" data-nyu-rmp-version="0.1.11"></div>
        </div>
      </div>
    `;
    const lookupProfessor = vi.fn(async () => null);
  ...
  it("replaces stale processed Albert rating roots with the score-first quick view", async () => {
    document.body.innerHTML = `
      <div role="row">
        <div role="gridcell" id="grid-instructor" data-nyu-rmp-processed="true">
          <div class="nyu-rmp-albert-original" data-nyu-rmp-original="true">Ada Lovelace</div>
          <div class="nyu-rmp-rating-root is-cell-mounted" data-nyu-rmp-version="0.1.11">
            <article class="nyu-rmp-card rating-good" data-nyu-rmp-requested-name="Ada Lovelace" data-nyu-rmp-version="0.1.11">
    ...
    expect(ratingRoot.dataset.nyuRmpVersion).toBe("0.1.11");
